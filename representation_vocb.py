...
class assert_circuits_equal_output(nn.Module):
    ...
    def forward(self,inputs):
        ...
        for i, (block, layer_past) in enumerate(zip(self.layers, past_key_values)):
            ...
            Output_mlp1_act=Act_mlp(Output_mlp1) #activated
            circuit_3=torch.matmul(Output_mlp1_act,W_mlp2)#R^[B,N,d]=[1,14,768]
            
            
            
            
            #circuit_4 is the attention+mlp path, attention_weight is as the same as one in circuit_2, but OVpath differs 
            circuit4_input_ln = block.ln_1(circuit_input)# make representation matrix get normed R^[N,d]=[14,768]
            ...
            #get circuit sum 
            circuit_sum=circuit_1+circuit_2+circuit_3+circuit_4 #R^[B,N,D]=[1,14,768]
