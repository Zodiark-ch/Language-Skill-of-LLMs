...
directory = 'json_logs/token_by_token/gpt2xl/orcainductive_cluster0'
circuit_layer=29
circuit_num=12*29    
...
if args.task_name=='language_skill':
    logger = get_logger('logs/' +'induction_logging.log')
    ...
    for filename in tqdm(os.listdir(directory)):
        ...
        if filename.endswith('.json'):
            ...
            for t in range(circuit_length):
                ...
                if t==0:  
                    inputs['input_ids']=input_ids_ori[:,:token_length-1]
                    inputs['attention_mask']=attention_mask_ori[:,:token_length-1]
                    outputs = orig_model(**inputs, labels=inputs["input_ids"])
                    _,label_ids=torch.topk(outputs.logits[0][-1],1)  
                    inputs['input_ids']=torch.cat((input_ids_ori[:,:token_length-1],label_ids.unsqueeze(0)),dim=-1)
                    inputs['attention_mask']=attention_mask_ori
                    
                if t==1:
                    inputs['input_ids']=input_ids_ori
                    inputs['attention_mask']=attention_mask_ori
    ...
    skill_topk=torch.where(skill_metrix>0.5,skill_metrix,0)
    ...
    # case_num=0
    # new_directory_cluster0='json_logs/token_by_token/gpt2xl/orcainductive_cluster0'
    # new_directory_cluster1='json_logs/token_by_token/gpt2xl/orcainductive_cluster1'
    ...
    #                 continue
    #         # open the JSON file
    #         if y_kmeans[case_num]==0:
    #             shutil.copy(fullpath, new_c0)
    #         if y_kmeans[case_num]==1:
